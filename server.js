...
exports.handleRequest = function(
	...
	req, 
	res, 
	proxy) { // proxy argument from require('http-proxy').createServer(function(req, res, proxy) { ... }) 
	if(port != 80) { subProxyHost += ':' + port; }	
	if(req.headers.host.indexOf(subProxyHost) == req.headers.host.length - subProxyHost.length) {
		...
		res.end('Internal Server Error: failed to identify host "' + req.headers.host + '"; expecting something ending with "' + subProxyHost +'"');
	}
};
